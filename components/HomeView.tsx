...
// ==========================================================================
//  홈 화면의 본체. 화면 파일이 아니라 컴포넌트인 이유 —
//
//  🔴같은 홈을 **두 곳**이 그린다.
//    · /      → subscriptionLive = SUBSCRIPTION_LIVE (지금은 false, 건당결제)
//    · /main  → subscriptionLive = true  (구독을 팔던 시절의 홈 그대로. PG 심사용)
//
//  🔴그래서 구독표를 /main 에 **복사하지 않는다.** 복사하면 심사에 낸 화면과
//    실제로 열 화면이 갈라져, 구독을 여는 날 둘이 어긋난 채로 나간다.
//    "출처가 둘이면 반드시 어긋난다"는 이 저장소의 원칙이 여기에도 그대로 걸린다.
//
//  ⚠️subscriptionLive 는 **화면을 어떻게 그릴까**만 정한다. 진짜로 파느냐는
//    여전히 lib/interim.ts 의 SUBSCRIPTION_LIVE 하나가 정한다 — /subscribe ·
//    /account 는 그 값에 따라 next.config.ts 가 막고 있다.
// ==========================================================================
...
export default function HomeView({ subscriptionLive }: { subscriptionLive: boolean }) {
  ...
  //   제품 칸의 archiMap · Colorgram, 히어로의 archiMap 장, 상단 막대의 [로그인]을
  //   싣지 않는다.
  //   🔴/main 에서는 **하나도 빼지 않는다.** 지금 PG사와 정기결제를 논의 중이고,
  //     /main 이 그 논의에 걸린 "나중에 쓸 사이트"다 — 함부로 바꾸지 말 것.
  //   ⚠️가르는 값이 preview 가 아니라 subscriptionLive 인 이유: preview 로 가르면
  //     구독을 여는 날(SUBSCRIPTION_LIVE=true) /main 의 모습까지 같이 사라진다.
  //     subscriptionLive 로 가르면 그날 홈이 저 모습을 그대로 물려받는다.
  const slides = subscriptionLive ? heroSlides : heroSlides.slice(0, 1);
  ...
  return (
    <main style={{
      ...
      <nav style={{
        ...
        <div className="main-nav-inner" style={{
          ...
          <div className="hnav-links" style={{ display: "flex", alignItems: "center", gap: "2px" }}>
            ...
            {/* 🔴로그인은 구독을 파는 홈(/main, 그리고 나중의 /)에만 남긴다 —
                지금 홈에서는 뺐다(2026-08-24 사용자 결정). 구독을 누르기 전에
                로그인할 문이 없으면 /main 이 막힌다.
                ⚠️다른 화면들의 막대(SiteHeader)에는 이 자리가 아예 없다 — 지금 홈에
                  되돌릴 때는 그쪽도 함께 넣어야 화면을 옮길 때 문이 안 사라진다. */}
            {subscriptionLive && <AuthNavLink />}
          </div>
        </div>
      </nav>
      ...
      <section className="main-hero" style={{
        ...
        {/* Slide dots + 정지 버튼. 한 장뿐이면 누를 것도 볼 것도 없다. */}
        {slides.length > 1 && (
      ...
      <section id="features" className="main-features" style={{ maxWidth: "1200px", margin: "0 auto", padding: "88px 48px 80px" }}>
        ...
        <div className="product-cards">
          ...
          {/* 🔴이 두 칸은 구독을 파는 홈(/main, 그리고 나중의 /)에만 남는다 —
              지금 홈(/)은 LaserFish 하나만 소개한다(2026-08-24 사용자 결정).
              ⚠️칸이 사라지면 product 를 "archimap" 으로 바꿀 길도 없어지므로,
                아래 기능 탭·요금 구역의 archiMap 갈래는 지금 홈에서 그려지지 않는다.
                되살리려면 여기 조건만 지우면 셋 다 함께 돌아온다. */}
          {subscriptionLive && (<>
      ...
      <section id="pricing" className="main-pricing" style={{
        ...
        <div style={{ maxWidth: subscriptionLive ? "920px" : "600px", margin: "0 auto" }}>
          <h2 style={{
            fontSize: "2.25rem",
            fontWeight: 900,
